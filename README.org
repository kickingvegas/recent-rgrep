...
Use the target ~install~ to install the Bash script ~recent-rgrep~ and Elisp script ~recent-rgrep.el~. 

#+begin_src shell
 $ make install 
#+end_src

The installation paths are described by the table below:

| Make Variable | Default Value | Description                                   |
|---------------+---------------+-----------------------------------------------|
| ~INSTALL_DIR~   | ~$(HOME)/bin~   | Installation destination for ~recent-rgrep~.    |
| ~ELISP_DIR~     | ~$(HOME)/emacs~ | Installation destination for ~recent-rgrep.el~. |

Edit or override ~INSTALL_DIR~ and ~ELISP_DIR~ to preference.
